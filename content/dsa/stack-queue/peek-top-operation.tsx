...
function simulatePeek(): PeekStep[] {
  ...
  function peek(lineIdx: number) {
    steps.push({ type: "check", top, val: null, stack: [...s], message: `Attempting to peek. Checking if IsEmpty().`, line: lineIdx });
    
    if (top === -1) {
      steps.push({ type: "underflow", top, val: null, stack: [...s], message: `Stack Underflow! Top is -1. Cannot peek.`, line: lineIdx + 1 });
      return;
    }

    const val = s[top];
    steps.push({ type: "read", top, val, stack: [...s], message: `Stack is not empty. Reading element at index ${top}.`, line: lineIdx + 4 });
    
    steps.push({ type: "done", top, val, stack: [...s], message: `Peek successful. Element ${val} read without modifying the Stack.`, line: lineIdx + 5 });
  }
